...
namespace satp
{
  dce_l1_quot_engine::dce_l1_quot_engine () :
      timer_notifier (smart_utils::timer_notifier::ETT_MONOTONIC, 5, 0), conn_state_ (
	  CONN_CLOSED), is_logged_ (false)
  {
  ...
  int_fast8_t
  dce_l1_quot_engine::init (const engine_info_t &ei)
  {
    engine_info_ = ei;
...
//	}
  ...
  void
  dce_l1_quot_engine::done ()
  {
    llrb_.reader_commit_data (1);
  }

  int
  dce_l1_quot_engine::onRspQuotUserLogout (
      UINT4 nSeqNo, const _fldRspMsg& rspmsg,
      const _fldTraderLogoutRsp& traderlogoutrsp, BYTE bChainFlag)
  {
    LOGGER()->info ("Got quot user logout rsp: %d, %s, %s\n",
		    const_cast<_fldRspMsg&> (rspmsg).ErrCode,
		    const_cast<_fldRspMsg&> (rspmsg).RspMsg.getValue (),
		    const_cast<_fldRspMsg&> (rspmsg).TimeStamp.getValue ());
    return 0;
  }

  int
  dce_l1_quot_engine::onMarketDataMBLQuot (UINT4 nSeqNo,
					   ...
					   BYTE bChainFlag)
  {
    ...
    memcpy (evt->body_.laq_.tm_,
	    const_cast<_fldBestQuot&> (bestquot).GenTime.getValue (),
	    const_cast<_fldBestQuot&> (bestquot).GenTime.Length ());
    evt->body_.laq_.contract_id_ = hash_str (
	const_cast<_fldBestQuot&> (bestquot).ContractID.getValue ());
  ...
  int
  dce_l1_quot_engine::onMarketDataArbiMBLQuot (
      UINT4 nSeqNo, const _fldArbiBestQuot& arbibestquot,
      CAPIVector<_fldMBLQuot>& lstMBLQuot, BYTE bChainFlag)
  {
    ...
    evt->id_ = EVT_L1_ARBI_QUOT;
    memcpy (evt->body_.laq_.tm_,
	    const_cast<_fldArbiBestQuot&> (arbibestquot).GenTime.getValue (),
	    const_cast<_fldArbiBestQuot&> (arbibestquot).GenTime.Length ());
    evt->body_.laq_.contract_id_ =
	hash_str (
	    const_cast<_fldArbiBestQuot&> (arbibestquot).ArbiContractID.getValue ());
  ...
  int
  dce_l1_quot_engine::onNtyCloseMktNotice (
      UINT4 nSeqNo, const _fldMktDataNotice& mktdatanotice, BYTE bChainFlag)
  {
    LOGGER()->info (
	"got market close msg: %s, %s, %s",
	const_cast<_fldMktDataNotice&> (mktdatanotice).Content.getValue (),
	const_cast<_fldMktDataNotice&> (mktdatanotice).Time.getValue (),
	const_cast<_fldMktDataNotice&> (mktdatanotice).Type.getValue ());
    return 0;
  }

  int
  dce_l1_quot_engine::onInvalidPackage (UINT4 nTID, WORD nSeries,
					UINT4 nSequenceNo, WORD nFieldCount,
					WORD nFieldsLen, const char* pAddr)
  {
    LOGGER()->error ("invalid package: %u, %u, %u, %u, %u, %s\n", nTID, nSeries,
		     nSequenceNo, nFieldCount, nFieldsLen, pAddr);
    LOGGER()->flush ();
    SU_ASSERT(false);
    return 0;
  }
  ...
  void
  dce_l1_quot_engine::onChannelLost (const char* szErrMsg)
  {
    LOGGER()->error ("quot conn closed:%s, %s, %s\n", member_id_.c_str (),
		     trader_id_.c_str (), szErrMsg);
    conn_evt_notifier_.notify (CONN_CLOSED);
  }

  void
  dce_l1_quot_engine::handle_timeout (uint64_t times)
  {
    ...
    strncpy (session_req.MemberID.buf, member_id_.c_str (),
	     session_req.MemberID.Length () - 1);
    strncpy (session_req.TraderNo.buf, trader_id_.c_str (),
	     session_req.TraderNo.Length () - 1);
    session_req.IsShortCert = '0';
    strncpy (session_req.ProgramID.buf, program_id_.c_str (),
	     session_req.ProgramID.Length () - 1);               //V1.0.8.0
    strncpy (session_req.ProgramVer.buf, program_ver_.c_str (),
	     session_req.ProgramVer.Length () - 1);               //V1.0.8.0
    ret = TraderSession (&session_req, (char *) passwd_.c_str (), &rsp,
			 &session_rsp);
    if (0 != ret)
      {
    ...
    strncpy (loginreq.MemberID.buf, member_id_.c_str (),
	     loginreq.MemberID.Length () - 1);
    strncpy (loginreq.TraderNo.buf, trader_id_.c_str (),
	     loginreq.TraderNo.Length () - 1);
    strncpy (loginreq.Pwd.buf, passwd_.c_str (), loginreq.Pwd.Length () - 1);
    strncpy (loginreq.AppName.buf, program_id_.c_str (),
	     loginreq.AppName.Length () - 1);   //V1.0.8.0
    strncpy (loginreq.AppVersion.buf, program_ver_.c_str (),
	     loginreq.AppVersion.Length () - 1);   //V1.0.8.0
