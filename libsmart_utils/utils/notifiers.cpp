	...
	if (!tmp_remove_notifiers_.empty())
	{
				...
				iter != TmpVec.end(); iter++)
		{
			...
			(*iter)->on_removed(
					0 == st ?
							false :
							(epoll_ctl(epfd_, EPOLL_CTL_DEL, (*iter)->get_fd(),
							NULL) == 0));
		}
	}
	...
	if (!tmp_add_notifiers_.empty())
	{
				...
				iter != tmp_add_notifiers.end(); iter++)
		{
			...
			(*iter)->on_added(
					(!ret.second) ?
							false :
							(epoll_ctl(epfd_, EPOLL_CTL_ADD, (*iter)->get_fd(),
									&evt) == 0));//you see, i wanna you get the errno:)...
		}
	}
...
timer_base::timer_base(const ETimerType tt, int64_t interval_seconds,
		int64_t interval_nanos) :
		fd_(-1), timer_type_(tt), init_expire_s_(interval_seconds), init_expire_ns_(
				interval_nanos), interval_s_(interval_seconds), interval_ns_(
				interval_nanos)
{
...
int32_t timer_base::open()
{
	...
	int32_t timer_type =
			ETT_REALTIME == timer_type_ ? CLOCK_REALTIME : CLOCK_MONOTONIC;
	...
	{ 0 };
	new_value.it_value.tv_sec = now.tv_sec + init_expire_s_
			+ (now.tv_nsec + init_expire_ns_) / NANOS_OF_ONE_SECONDS;
	new_value.it_value.tv_nsec = (now.tv_nsec + init_expire_ns_)
			% NANOS_OF_ONE_SECONDS;
	new_value.it_interval.tv_sec = interval_s_
			+ interval_ns_ / NANOS_OF_ONE_SECONDS;
...
{
	return (fd_ = eventfd(0, 0)) == -1 ? SU_EC_ERR : SU_EC_SUC;
}
...
tcp_sock_accepter::tcp_sock_accepter(std::string &bind_ip,
		uint_fast16_t bind_port) :
...
void tcp_sock_accepter::handle_events(uint32_t evts)
{
	...
	handle_new_conn(sock, std::string(inet_ntoa(addr.sin_addr)),
			ntohs(addr.sin_port));
}
